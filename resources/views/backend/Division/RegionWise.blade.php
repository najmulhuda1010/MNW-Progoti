                ...
                <div class="col-md-12">
                  ...
                  $region = DB::select(DB::raw("select region_id from mnw_progoti.monitorevents where division_id='$division' and year='$year' and quarterly='$quarter' group by region_id order by region_id ASC"));
                      foreach($region as $row)
                      {
                         ...
                         $dataget = DB::select(DB::raw("select * from mnw_progoti.monitorevents where region_id='$region_id' and year='$year' and quarterly='$quarter'"));
                     ...
                     <table style="text-align: center;font-size:13" id="<?php echo "dv".$ct; ?>" class="table" cellspacing="0" width="100%">
                      ...
                      <tbody>
                        ...
                        foreach ($area as $row) 
                        {
                          ...
                          $totalspoint=0;
                          $totalsectp =0;
                          $eventid = DB::select(DB::raw("select * from mnw_progoti.monitorevents where region_id='$region_id' and year='$year' and quarterly='$quarter' and area_id='$areaid'"));
                          if(!empty($eventid))
                          {
                        ...
                        ?>
                        <tr>
                          <td width="10%" style="text-align:center;"><button class="btn btn-light" onClick="sectionDiv(<?php echo $event_id; ?>,<?php echo $section; ?>);"><?php echo $areaname; ?></button></td>
